...
def plot(path: str, significant: bool, alldata: bool, plot_choice: str):
    ...
    if alldata:
        ...
        indend = df[(df['Altitude'] < df.at[indstart + 3000, 'Altitude']) & (df['Stage'] == Stage.DESCENT)].index[0] + 3000
    ...
    if plot_choice=="Altitude":
        ...
        plt.plot(df.iloc[rangevals, df.columns.get_loc("Time")], 
                 df.iloc[rangevals, df.columns.get_loc("Velocity_Z")], 
                 color = "red")
        plt.xlabel("Time (s)")
        ...
        plt.plot(df.iloc[rangevals, df.columns.get_loc("Time")], 
                 df.iloc[rangevals, df.columns.get_loc("Acceleration_Z")], 
                 color = "red")
        plt.xlabel("Time (s)")
        ...
        plt.plot(df.iloc[rangevals, df.columns.get_loc("Time")], 
                 df.iloc[rangevals, df.columns.get_loc("Servo_Angle")], 
                 color = "red")
        plt.xlabel("Time (s)")
