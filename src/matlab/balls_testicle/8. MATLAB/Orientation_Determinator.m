...
%% Create Data Vectors
AccelerometerBNO085 = [launch_data.Acceleration_X_BNO085, launch_data.Acceleration_Y_BNO085, launch_data.Acceleration_Z_BNO085]; % *32.17/9.81; % shouldn't thrust be positive and gravity negative?
...
for i = 1:length(launch_data.Time)-1
    ...
    % quaternion_calculated(i+1,:) = madgwickFilter(quaternion_calculated(i,:), GyroscopeBNO085(i,:), AccelerometerBNO085(i,:), dt, 0.05); % use updateQuaternion, mahonyFilter, or madgwickFilter
end
...
xline(launch_data.t_apogee, 'LineWidth',sv.LineWidth2,'Color',sv.Black, 'DisplayName', 'Apogee')
xlabel('Time [s]'); ylabel('Zenith Angle, Z');
legend('Location','northeast')
...
yline(1.1*g, '--','LineWidth',sv.LineWidth1,'Color',sv.Black, 'DisplayName', 'Gravity Threshold')
yline(0.9*g, '--','LineWidth',sv.LineWidth1,'Color',sv.Black, 'HandleVisibility', 'off')
xlabel('Time [s]'); ylabel('Acceleration [ft/s^2]');
title('Flight Data')
legend('Location','northwest')