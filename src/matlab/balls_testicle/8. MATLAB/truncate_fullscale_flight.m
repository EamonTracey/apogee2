...
%% Find start and end of relevant flight data
    ...
    if data.Servo_Angle(1) == -5
        data.Servo_Angle = data.Servo_Angle + 5;
    end

    for i = 1:n
        if abs(data.Acceleration_Z_ICM20649(i))*3.280839895>8*g && truth1 == 1
            ilaunch = i;
            truth1 = 0;
        end
        ...
        if data.Servo_Angle(i) ~= 0 && truth2 == 1
            iflap = i+1;
            truth2 = 0;
        end
...
%% Truncate data 
    ...
    launch_data.Time_Raw = data.Time(1:iend);
    launch_data.dt = dt;
    ...
    launch_data.Altitude_Raw = data.Altitude_BMP390(1:iend)*3.280839895;
    launch_data.Altitude = data.Altitude(ibeg:iend); % *3.280839895;
    ...
    % Predicted Apogee
    % disp(data)
    % if isfield(data, "Predicted_Apogee")
        launch_data.Predicted_Apogee = data.Predicted_Apogee(ibeg:iend);
...
%% Save Variables in launch_data
    launch_data.Apogee = apogee*3.280839895 - mean(launch_data.Altitude_AGL(1:round(1.5*freq)));
