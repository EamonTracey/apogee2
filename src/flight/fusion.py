...
class FusionComponent(Component):
    ...
    def teasleyFilter(self, quat, gyro, dt):
        ...
        # Normalize 
        qnewer = tuple(x / np.linalg.norm(quat) for x in qnew) if np.linalg.norm(quat) != 0 else qnew
    ...
    def dispatch(self, time: float):
        ...
        # NOTE: Gyro drift starts to kick in pretty heavily after around 15-20 seconds.
            ...
            # Calculate euler angles
            self._state.euler = tuple(x * (180/pi) for x in quatern2euler(self._state.quaternion))
