...
def calculate_derivatives_eamonteasley(vehicle, motor, environment,
                                       angle_of_actuation, time, position,
    ...
    body_to_earth = Rotation.from_quat(np.roll(orientation, -1))
    ...
    velocity_relative_air_magnitude = np.round(
        np.linalg.norm(velocity_relative_air_earth), decimals=8)
    ...
    if velocity_relative_air_magnitude != 0:
        ...
        cosine_angle_of_attack = np.dot(roll_earth,
                                        velocity_relative_air_direction_earth)
        angle_of_attack = np.round(np.arccos(
            np.clip(cosine_angle_of_attack, -1, 1)),
                                   decimals=8)
    else:
        angle_of_attack = 0
    # TODO? #
    ...
    force_axial_body = np.array([
        -vehicle.calculate_axial_force(
            angle_of_actuation, abs(angle_of_attack), mach_number, density), 0,
        0
    ])
    ...
    if angle_of_attack != 0:
        force_normal_magnitude = vehicle.calculate_normal_force(
            angle_of_actuation, abs(angle_of_attack), mach_number, density)
        velocity_relative_air_body = earth_to_body.apply(
            velocity_relative_air_earth)
        velocity_relative_air_normal_body = np.insert(
            velocity_relative_air_body[1:], 0, 0)
        force_normal_direction_body = -velocity_relative_air_normal_body / np.linalg.norm(
            velocity_relative_air_normal_body)
    else:
    ...
    # The torque on the vehicle equals the torque caused by the difference
    # between the center of mass and the center of pressure plus the damping
    # torque. Rather than compute a damping torque, we estimate it with a 0.2
    # multiplier.
    moment_arm_body = np.array(
        [vehicle.center_of_mass - vehicle.center_of_pressure, 0, 0])
    torque_body = np.cross(moment_arm_body, force_normal_body)
    torque_body *= 0.2

    # Newton's Second Law T=Ia
    angular_acceleration = torque_body / vehicle.inertia
